...
def encrypt_message_gui():
    ...
    # Display the encrypted message and metadata
    encrypted_message_entry.delete('1.0', END)
    encrypted_message_entry.insert('1.0', ciphertext.hex())
    nonce_entry.delete(0, END)
    nonce_entry.insert(0, nonce.hex())
    tag_entry.delete(0, END)
    tag_entry.insert(0, tag.hex())

def decrypt_message_gui():
    ciphertext_hex = encrypted_message_entry.get('1.0', END).strip()
    if ciphertext_hex == "":
        messagebox.showerror("Error", "Please enter an encrypted message to decrypt.")
        return

    # Get the nonce and tag from the GUI
    nonce = bytes.fromhex(nonce_entry.get())
    tag = bytes.fromhex(tag_entry.get())
